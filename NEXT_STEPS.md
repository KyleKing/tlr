...
- Issue fetcher. `deno task issues "<project>"` (`scripts/issues.ts`) resolves a project by name or
  slug against the Linear GraphQL API and refreshes `project`, `cycles`, `currentCycle`, `milestones`,
  and `issues` in the data file from live data, replacing the deleted Python script for that job.
  `capacity`/`teamVelocity`/`teamCapacityPerCycle` are untouched (owned by `capacity`/`roster`). A
  milestone's key is the part of its Linear name before the colon (`"M1: Measure and page"` → `M1`);
  its `progress` comes straight from Linear's own `ProjectMilestone.progress` rather than being
  recomputed locally. Pure transforms in `web/lib/issues.js` (tested), thin I/O in `scripts/issues.ts`.
...
2. **Project switcher.** Today the web app only ever reads one static `/data/cpu.json`; `deno task
   issues` refreshes it by name or slug, but there's still no concept of "list the projects I'm a
   member of" anywhere. Needs: a Linear GraphQL query for the viewer's member projects (listing rather
   than searching), a per-project data file convention (e.g. `web/data/<slug>.json`) so switching is
   instant client-side, and a picker in the header (a natural fit for the command palette from item 1,
   if that lands first — "switch to project" as one of its actions).
